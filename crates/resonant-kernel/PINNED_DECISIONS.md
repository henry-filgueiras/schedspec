...
| P21 | A node that had not yet seen an operator override merged its stale `disputed` view against the overridden `quarantined` one at the same epoch — an honest same-epoch dispute that silently overturned the operator's visible decision | An override is fresh authoritative evidence: it applies at an epoch strictly ahead of everything known about the subject (`OperatorOverride.epoch`, carried on the wire so every applier lands identically), and state confirmations absorb the deciding epoch too. Merges against stale views then resolve through freshness dominance — the race stays visible as residue, but the operator's decision is never re-disputed by evidence that predates it. |
