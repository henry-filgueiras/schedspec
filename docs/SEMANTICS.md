...
- residue identity must be content-addressed on the subject of the disagreement — the belief key (scope, subject) — never on the minting event. Replica-local counters make two replicas performing the same reunion mint different residue identities; even shared reunion coordinates are not enough once reunions cascade pairwise (one pair reunites first, so the next reunion observes the same conflict through a different tension shape). Identity names the conflict; the tension sides, detail, and birth coordinates are content that a fresh observation may visibly supersede.
- residue tracks *live* disagreement: a later stable, conflict-free convergence on a subject resolves its residue into the visible resolved record, with the merge that settled it as evidence. Preserved disagreement that outlives the disagreement itself is cognitive debt, not honesty — but resolution moves history aside legibly, it never erases it.
