...
- **Resolution:** residue identity is content-addressed on the subject of the disagreement — the belief key (scope, subject) — never on the minting event. Shared reunion coordinates (P18) turned out to be necessary but not sufficient: over a real transport, reunions cascade pairwise, so the same conflict is observed through different tension shapes and coordinate-derived identities still diverge. Identity names the conflict; sides, detail, and birth coordinates are visible content a fresh observation may supersede, and a later stable convergence resolves the residue into the visible record. `SEMANTICS.md` states this under "Refinements From The Reference Kernel"; the kernel pins it as P20 and the wire integration test (`crates/resonant-node/tests/split_brain.rs`) demonstrates convergence over real libp2p transport.
- **Status:** resolved
